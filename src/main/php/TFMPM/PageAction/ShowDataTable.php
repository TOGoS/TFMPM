...
class TFMPM_PageAction_ShowDataTable extends TFMPM_PageAction_TemplatePageAction
{
	protected $rc;
	protected $items;
	protected $postFormInfo;
	
	public static function getInstance( TFMPM_Registry $reg, EarthIT_Schema_ResourceClass $rc, $items=null ) {
		$collectionName =
			$rc->getFirstPropertyValue(EarthIT_CMIPREST_NS::COLLECTION_NAME) ?:
			EarthIT_Schema_WordUtil::pluralize($rc->getName());
		
		$className = 'TFMPM_PageAction_Show'.EarthIT_Schema_WordUtil::toPascalCase($collectionName);
		if( !class_exists($className) or !is_subclass_of($className, 'TFMPM_PageAction_ShowDataTable') ) {
			$className = 'TFMPM_PageAction_ShowDataTable';
		}
	 ...
	 * template responses)
	 */
	public function __construct( TFMPM_Registry $reg, EarthIT_Schema_ResourceClass $rc, $items=null, array $postFormInfo=null ) {
	...
	public function getTemplateParameters() {
		...
		$postForm = new TFMPM_FormBlob($formInfo, array(
