...
class PHPTemplateProjectNS_AS_Script implements PHPTemplateProjectNS_AS_Action
{
	protected $actions;
	...
	public static function parse( $script, $actionTable ) {
		...
		foreach( $tokens as $tok ) {
			if( isset($actionTable[$tok]) ) {
				...
				$act = new PHPTemplateProjectNS_AS_PushValue($tok);
			} else if( preg_match('/^"([^\\"]+)"$/', $tok, $bif) ) {
				$act = new PHPTemplateProjectNS_AS_PushValue($bif[1]);
			} else {
				throw new Exception("Unrecognized word: $tok");
			}
			$actions[] = $act;
		}
		return new PHPTemplateProjectNS_AS_Script($actions);
	}
